...
typedef struct {
    uint32_t
        Prescaler; /*!< Specifies the prescaler value used to divide the TIM clock.
                      ...
                      0xFFFF Macro __HAL_TIM_CALC_PSC() can be used to calculate prescaler value */
    ...
    uint32_t Period; /*!< Specifies the period value to be loaded into the active
                          ...
                          This parameter can be a number between Min_Data = 0x0000 and Max_Data =
                        0xFFFF (or 0xFFEF if dithering is activated)Macros __HAL_TIM_CALC_PERIOD(),
                           __HAL_TIM_CALC_PERIOD_DITHER(),__HAL_TIM_CALC_PERIOD_BY_DELAY(),
                           __HAL_TIM_CALC_PERIOD_DITHER_BY_DELAY()can be used to calculate Period
                        value */
...
typedef struct {
    ...
    uint32_t Pulse; /*!< Specifies the pulse value to be loaded into the Capture Compare Register.
                         This parameter can be a number between Min_Data = 0x0000 and Max_Data =
                       0xFFFF (or 0xFFEF if dithering is activated) Macros __HAL_TIM_CALC_PULSE(),
                       __HAL_TIM_CALC_PULSE_DITHER() can be used to calculate Pulse value */
...
#define TIM_ENCODERMODE_TI12                                                                  \
    ...
    ) /*!< Quadrature encoder mode 3, x4 mode, counts up/down on both TI1FP1 and TI2FP2 edges \
...
#if defined(TIM20)
#define TIM_TS_ITR9 \
    (TIM_SMCR_TS_0 | TIM_SMCR_TS_2 | TIM_SMCR_TS_3) /*!< Internal Trigger 9 (ITR9)              */
#endif                                              /* TIM20 */
#define TIM_TS_ITR10 \
...
#define TIM_BREAK_SYSTEM_PVD                                                                  \
    SYSCFG_CFGR2_PVDL /*!< Enables and locks the PVD connection with TIM1/8/15/16/17/20 Break \
                         Input and also the PVDE and PLS bits of the Power Control Interface */
#define TIM_BREAK_SYSTEM_SRAM_PARITY_ERROR                                                   \
    SYSCFG_CFGR2_SPL /*!< Enables and locks the SRAM_PARITY error signal with Break Input of \
                        TIM1/8/15/16/17/20 */
...
HAL_StatusTypeDef HAL_TIM_Base_Start_DMA(
    TIM_HandleTypeDef *htim, const uint32_t *pData, uint16_t Length
);
