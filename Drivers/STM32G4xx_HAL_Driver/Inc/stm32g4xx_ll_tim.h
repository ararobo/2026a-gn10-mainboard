...
#define LL_TIM_UPDATESOURCE_REGULAR                                                              \
    0x00000000U /*!< Counter overflow/underflow, Setting the UG bit or Update generation through \
                   the slave mode controller generates an update request */
...
#define LL_TIM_COUNTERMODE_CENTER_UP_DOWN                                                         \
    TIM_CR1_CMS /*!< The counter counts up and down alternatively. Output compare interrupt flags \
                   of output channels  are set only when the counter is counting up or down. */
/**
 * @}
 */
...
#define LL_TIM_OCMODE_PWM1                                                                    \
    ...
    ) /*!<In upcounting, channel y is active as long as TIMx_CNT<TIMx_CCRy else inactive.  In \
...
#define LL_TIM_OCMODE_PWM2                                                                    \
    ...
    ) /*!<In upcounting, channel y is inactive as long as TIMx_CNT<TIMx_CCRy else active.  In \
...
#define LL_TIM_OCMODE_COMBINED_PWM1 (TIM_CCMR1_OC1M_3 | TIM_CCMR1_OC1M_2) /*!<Combined PWM mode \
                                                                             1*/
...
#define LL_TIM_ENCODERMODE_X4_TI12                                                               \
    ...
    ) /*!< Quadrature encoder mode 3, x4 mode - Counter counts up/down on both TI1FP1 and TI2FP2 \
...
#define LL_TIM_ENCODERMODE_DIRECTIONALCLOCK_X1_TI12                                               \
    ...
    ) /*!< Encoder mode: Directional Clock, x1 mode, TI1FP1 and TI2FP2 edge sensitivity is set by \
         CC1P and CC2P */
#define LL_TIM_ENCODERMODE_X1_TI1                                                               \
    (TIM_SMCR_SMS_3 | TIM_SMCR_SMS_2 | TIM_SMCR_SMS_1                                           \
    ) /*!< Quadrature encoder mode: x1 mode, counting on TI1FP1 edges only, edge sensitivity is \
         set by CC1P */
#define LL_TIM_ENCODERMODE_X1_TI2                                                               \
    (TIM_SMCR_SMS_3 | TIM_SMCR_SMS_2 | TIM_SMCR_SMS_1 | TIM_SMCR_SMS_0                          \
    ) /*!< Quadrature encoder mode: x1 mode, counting on TI2FP2 edges only, edge sensitivity is \
         set by CC1P */
/**
 * @}
 */
...
#define LL_TIM_SLAVEMODE_COMBINED_RESETTRIGGER                                                    \
    TIM_SMCR_SMS_3 /*!< Combined reset + trigger mode - Rising edge of the selected trigger input \
...
#define LL_TIM_SLAVEMODE_COMBINED_GATEDRESET                                                     \
    (TIM_SMCR_SMS_3 | TIM_SMCR_SMS_0                                                             \
         ...
         (TRGI) is high. The counter stops and is reset) as soon as the trigger becomes low.Both \
         startand stop of the counter are controlled. */
/**
 * @}
 */
...
#define LL_TIM_TIM20_ETRSOURCE_ADC5_AWD1 \
    (TIM1_AF1_ETRSEL_3 | TIM1_AF1_ETRSEL_1 | TIM1_AF1_ETRSEL_0) /*!< ADC5 analog watchdog 1 */
...
#define LL_TIM_TIM20_ETRSOURCE_ADC5_AWD3 \
    (TIM1_AF1_ETRSEL_3 | TIM1_AF1_ETRSEL_2 | TIM1_AF1_ETRSEL_0) /*!< ADC5 analog watchdog 3 */
#endif                                                          /* ADC5 */
/**
 * @}
 */
...
#define LL_TIM_DMABURST_BASEADDR_CCR5                                                             \
    (TIM_DCR_DBA_4 | TIM_DCR_DBA_1) /*!< TIMx_CCR5 register is the DMA base address for DMA burst \
                                     */
...
#define LL_TIM_OCREF_CLR_INT_COMP6 \
    (TIM1_AF2_OCRSEL_2 | TIM1_AF2_OCRSEL_0) /*!< OCREF clear input is connected to COMP6_OUT */
#endif                                      /* COMP6 */
#if defined(COMP7)
#define LL_TIM_OCREF_CLR_INT_COMP7 \
    (TIM1_AF2_OCRSEL_2 | TIM1_AF2_OCRSEL_1) /*!< OCREF clear input is connected to COMP7_OUT */
#endif                                      /* COMP7 */
/**
 * @}
 */
 ...
 * achieve the required output signal frequency.
 ...
 * the required timer output compare active/inactive delay.
...
/**
 * @brief  HELPER macro calculating the auto-reload value, with dithering feature enabled, to
 * achieve the required pulse duration (when the timer operates in one pulse mode).
 ...
 *       In case dithering is activated,macro __LL_TIM_CALC_ARR_DITHER can be used instead, to
...
__STATIC_INLINE void LL_TIM_OC_SetIdleState(TIM_TypeDef *TIMx, uint32_t Channel, uint32_t IdleState)
{
    uint8_t iChannel = TIM_GET_CHANNEL_INDEX(Channel);
    MODIFY_REG(
        TIMx->CR2, (TIM_CR2_OIS1 << SHIFT_TAB_OISx[iChannel]), IdleState << SHIFT_TAB_OISx[iChannel]
    );
}
 ...
 * @note If dithering is activated, CompareValue can be calculated with macro @ref
...
__STATIC_INLINE void LL_TIM_ConfigETR(
    TIM_TypeDef *TIMx, uint32_t ETRPolarity, uint32_t ETRPrescaler, uint32_t ETRFilter
)
{
    MODIFY_REG(
        TIMx->SMCR,
...
__STATIC_INLINE void LL_TIM_ConfigBRK(
    TIM_TypeDef *TIMx, uint32_t BreakPolarity, uint32_t BreakFilter, uint32_t BreakAFMode
)
{
    MODIFY_REG(
        TIMx->BDTR,
...
__STATIC_INLINE void LL_TIM_ConfigBRK2(
    TIM_TypeDef *TIMx, uint32_t Break2Polarity, uint32_t Break2Filter, uint32_t Break2AFMode
)
{
    MODIFY_REG(
        TIMx->BDTR,
 ...
 * @retval None
 */
__STATIC_INLINE void LL_TIM_SetOffStates(
 ...
 * @retval None
 */
__STATIC_INLINE void LL_TIM_EnableBreakInputSource(
 ...
 * @retval None
 */
__STATIC_INLINE void LL_TIM_DisableBreakInputSource(
    TIM_TypeDef *TIMx, uint32_t BreakInput, uint32_t Source
)
{
    __IO uint32_t *pReg = (__IO uint32_t *)((uint32_t)((uint32_t)(&TIMx->AF1) + BreakInput));
...
__STATIC_INLINE void LL_TIM_SetBreakInputSourcePolarity(
    TIM_TypeDef *TIMx, uint32_t BreakInput, uint32_t Source, uint32_t Polarity
)
{
    __IO uint32_t *pReg = (__IO uint32_t *)((uint32_t)((uint32_t)(&TIMx->AF1) + BreakInput));
    MODIFY_REG(
        *pReg, (TIMx_AF1_BKINP << TIM_POSITION_BRK_SOURCE), (Polarity << TIM_POSITION_BRK_SOURCE)
    );
}
/**
...
/**
 * @brief  Set the falling edge dead-time delay (delay inserted between the falling edge of the
 * OCxREF signal and the rising edge of OCxN signals).
 * @note Macro IS_TIM_DEADTIME_ASYMMETRICAL_INSTANCE(TIMx) can be used to check whether or not
...
/**
 * @brief  Get the falling edge dead-time delay (delay inserted between the falling edge of the
 * OCxREF signal and the rising edge of OCxN signals).
 * @note Macro IS_TIM_DEADTIME_ASYMMETRICAL_INSTANCE(TIMx) can be used to check whether or not
 *       asymmetrical dead-time insertion feature is supported by a timer instance.
 * @note This bit-field can not be modified as long as LOCK level 1, 2 or 3 has been programmed
...
/**
 * @brief  Indicate whether transition error interrupt flag (TERRF) is set (transition error
 * interrupt is pending).
 * @note Macro IS_TIM_ENCODER_ERROR_INSTANCE(TIMx) can be used to check whether or not
 *       a timer instance provides encoder error management.
...
/**
 * @brief  Indicate whether index error interrupt flag (IERRF) is set (index error interrupt is
 * pending).
 * @note Macro IS_TIM_ENCODER_ERROR_INSTANCE(TIMx) can be used to check whether or not
 *       a timer instance provides encoder error management.
 ...
 * @note Macro IS_TIM_FUNCTINONAL_ENCODER_INTERRUPT_INSTANCE(TIMx) can be used to check whether or
 * not a timer instance provides encoder interrupt management.
...
/**
 * @brief  Indicate whether direction change interrupt flag (DIRF) is set (direction change
 * interrupt is pending).
 * @note Macro IS_TIM_FUNCTINONAL_ENCODER_INTERRUPT_INSTANCE(TIMx) can be used to check whether or
 * not a timer instance provides encoder interrupt management.
...
ErrorStatus LL_TIM_OC_Init(
    TIM_TypeDef *TIMx, uint32_t Channel, const LL_TIM_OC_InitTypeDef *TIM_OC_InitStruct
);
void LL_TIM_IC_StructInit(LL_TIM_IC_InitTypeDef *TIM_ICInitStruct);
ErrorStatus LL_TIM_IC_Init(
    TIM_TypeDef *TIMx, uint32_t Channel, const LL_TIM_IC_InitTypeDef *TIM_IC_InitStruct
);
