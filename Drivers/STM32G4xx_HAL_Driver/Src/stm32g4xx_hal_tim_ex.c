...
)
{
    ...
    if (IS_TIM_BKIN2_INSTANCE(htim->Instance)) {
        ...
        MODIFY_REG(
            tmpbdtr, TIM_BDTR_BK2F, (sBreakDeadTimeConfig->Break2Filter << TIM_BDTR_BK2F_Pos)
        );
 ...
 *           - CNT: only CNT[11:0] holds the non-dithered part for 16b timers (or CNT[26:0] for 32b
 * timers)
 *           - ARR: ARR[15:4] holds the non-dithered part, and ARR[3:0] the dither part for 16b
 * timers
 *           - CCRx: CCRx[15:4] holds the non-dithered part, and CCRx[3:0] the dither part for 16b
 * timers
 ...
 * @note   Macros @ref __HAL_TIM_CALC_PERIOD_DITHER() __HAL_TIM_CALC_DELAY_DITHER()
 * __HAL_TIM_CALC_PULSE_DITHER() can be used to calculate period (ARR) and delay (CCRx) value.
 * @note   Enabling dithering, modifies automatically values of registers ARR/CCRx to keep the same
 * integer part.
 * @note   Enabling dithering, modifies automatically values of registers ARR/CCRx to keep the same
 * integer part. So it may be necessary to read ARR value or CCRx value with macros @ref
 ...
 *             (corresponds to 4094 for the integer part and 15 for the dithered part).
 * @note   Disabling dithering, modifies automatically values of registers ARR/CCRx to keep the same
 * integer part. So it may be necessary to read ARR value or CCRx value with macros @ref
 * __HAL_TIM_GET_AUTORELOAD()
 *         __HAL_TIM_GET_COMPARE() and if necessary update Init structure field htim->Init.Period .
 * @retval HAL status
 ...
 *         This parameter can be one of the following values:
 *            @arg TIM_SMS_PRELOAD_SOURCE_UPDATE: Timer update event is used as source of Slave Mode
 * Selection preload
 *            @arg TIM_SMS_PRELOAD_SOURCE_INDEX: Timer index event is used as source of Slave Mode
 * Selection preload
 * @retval HAL status
 */
