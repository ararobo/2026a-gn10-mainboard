...
static HAL_StatusTypeDef I2C_Master_ISR_IT(
    struct __I2C_HandleTypeDef *hi2c, uint32_t ITFlags, uint32_t ITSources
);
static HAL_StatusTypeDef I2C_Mem_ISR_IT(
    struct __I2C_HandleTypeDef *hi2c, uint32_t ITFlags, uint32_t ITSources
);
static HAL_StatusTypeDef I2C_Slave_ISR_IT(
    struct __I2C_HandleTypeDef *hi2c, uint32_t ITFlags, uint32_t ITSources
);
static HAL_StatusTypeDef I2C_Master_ISR_DMA(
    struct __I2C_HandleTypeDef *hi2c, uint32_t ITFlags, uint32_t ITSources
);
static HAL_StatusTypeDef I2C_Mem_ISR_DMA(
    struct __I2C_HandleTypeDef *hi2c, uint32_t ITFlags, uint32_t ITSources
);
static HAL_StatusTypeDef I2C_Slave_ISR_DMA(
    struct __I2C_HandleTypeDef *hi2c, uint32_t ITFlags, uint32_t ITSources
);
...
static HAL_StatusTypeDef I2C_WaitOnSTOPFlagUntilTimeout(
    I2C_HandleTypeDef *hi2c, uint32_t Timeout, uint32_t Tickstart
);
static HAL_StatusTypeDef I2C_IsErrorOccurred(
    I2C_HandleTypeDef *hi2c, uint32_t Timeout, uint32_t Tickstart
);
...
)
{
    ...
    if (hi2c->State == HAL_I2C_STATE_READY) {
        ...
        if (hi2c->XferSize > 0U) {
             ...
             * RESTART */
            I2C_TransferConfig(
                hi2c, DevAddress, (uint8_t)(hi2c->XferSize + 1U), xfermode, I2C_GENERATE_START_WRITE
            );
        } else {
            /* Send Slave Address */
            /* Set NBYTES to write and reload if hi2c->XferCount > MAX_NBYTE_SIZE and generate
             * RESTART */
            I2C_TransferConfig(
                hi2c, DevAddress, (uint8_t)hi2c->XferSize, xfermode, I2C_GENERATE_START_WRITE
            );
        }
...
)
{
    ...
    if (hi2c->State == HAL_I2C_STATE_READY) {
        ...
        if (hi2c->XferCount > MAX_NBYTE_SIZE) {
            hi2c->XferSize = MAX_NBYTE_SIZE;
            I2C_TransferConfig(
                hi2c, DevAddress, (uint8_t)hi2c->XferSize, I2C_RELOAD_MODE, I2C_GENERATE_START_READ
            );
        } else {
            hi2c->XferSize = hi2c->XferCount;
            I2C_TransferConfig(
                hi2c, DevAddress, (uint8_t)hi2c->XferSize, I2C_AUTOEND_MODE, I2C_GENERATE_START_READ
            );
        }
...
)
{
    ...
    if (hi2c->State == HAL_I2C_STATE_READY) {
        ...
        if (hi2c->XferSize > 0U) {
            ...
            I2C_TransferConfig(
                hi2c, DevAddress, (uint8_t)sizetoxfer, I2C_AUTOEND_MODE, I2C_GENERATE_START_WRITE
            );
    ...
    if (hi2c->State == HAL_I2C_STATE_READY) {
        ...
        if (hi2c->XferSize > 0U) {
            if (hi2c->hdmarx != NULL) {
                ...
                /* Enable the DMA channel */
                dmaxferstatus = HAL_DMA_Start_IT(
                    hi2c->hdmarx, (uint32_t)&hi2c->Instance->RXDR, (uint32_t)pData, hi2c->XferSize
                );
            } else {
                /* Update I2C state */
                hi2c->State = HAL_I2C_STATE_READY;
...
)
{
    ...
    if (hi2c->State == HAL_I2C_STATE_READY) {
        ...
        /* Send Slave Address and Memory Address */
        I2C_TransferConfig(
            hi2c, DevAddress, (uint8_t)MemAddSize, I2C_RELOAD_MODE, I2C_GENERATE_START_WRITE
        );
...
)
{
    ...
    if (hi2c->State == HAL_I2C_STATE_READY) {
        ...
        if (hi2c->hdmatx != NULL) {
            ...
            /* Enable the DMA channel */
            dmaxferstatus = HAL_DMA_Start_IT(
                hi2c->hdmatx, (uint32_t)pData, (uint32_t)&hi2c->Instance->TXDR, hi2c->XferSize
            );
        } else {
            /* Update I2C state */
            hi2c->State = HAL_I2C_STATE_READY;
        ...
        do {
            ...
            if (__HAL_I2C_GET_FLAG(hi2c, I2C_FLAG_AF) == RESET) {
                ...
                if (I2C_WaitOnFlagUntilTimeout(hi2c, I2C_FLAG_STOPF, RESET, Timeout, tickstart) !=
                    HAL_OK) {
                    /* A non acknowledge appear during STOP Flag waiting process, a new trial must
                     * be performed */
                    ...
                    /* A acknowledge appear during STOP Flag waiting process, this mean that device
                     * respond to its address */
    ...
    if (hi2c->State == HAL_I2C_STATE_READY) {
        ...
        if (hi2c->XferSize > 0U) {
            ...
            if (dmaxferstatus == HAL_OK) {
                ...
                if ((XferOptions == I2C_FIRST_FRAME) || (XferOptions == I2C_FIRST_AND_LAST_FRAME)) {
                    I2C_TransferConfig(
                        hi2c, DevAddress, (uint8_t)sizetoxfer, xfermode, xferrequest
                    );
                } else {
                    I2C_TransferConfig(
                        hi2c, DevAddress, (uint8_t)hi2c->XferSize, xfermode, xferrequest
                    );
                }
 ...
 * @retval None
 */
__weak void HAL_I2C_AddrCallback(
...
)
{
    ...
    if ((I2C_CHECK_FLAG(ITFlags, I2C_FLAG_AF) != RESET) &&
        ...
        if (hi2c->XferCount != 0U) {
            ...
            I2C_TransferConfig(
                hi2c, devaddress, (uint8_t)hi2c->XferSize, xfermode, I2C_NO_STARTSTOP
            );
